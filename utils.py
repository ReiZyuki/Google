...
def set_result(target, value):
    if not isinstance(target, str): raise TypeError("Result variable name must be a string.")
    return value

def validate_target(target):
    if not isinstance(target, str): raise TypeError("Right-side value must be a variable name string.")
    ...
    try:
        ...
        while frame:
            ...
            if not filename.startswith(library_dir):
                ...
                try: frame.f_locals[target] = value
